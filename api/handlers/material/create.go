...
// CreateMaterial maneja la creación de un material con form-data (para imágenes/videos en Supabase Storage)
func CreateMaterial(c *gin.Context) {
	...
	herramientasStr := c.PostForm("herramientas") // e.g., '["tag1","tag2"]'
	composicionStr := c.PostForm("composicion")   // ídem
	derivadoDeStr := c.PostForm("derivado_de")    // UUID string
	creadorIDStr := c.PostForm("creador_id")      // string, convert to uint
	...
	galeriaCaptionsStr := c.PostForm("galeria_captions") // New: '["caption1", "caption2"]' for images in order
	...
	for i, fileHeader := range files {
		// Reemplazar espacios en filename para evitar problemas en URLs
